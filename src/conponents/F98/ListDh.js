...
const DonHangComponent = () => {
    ...
    const [soDonDaDuyet, setSoDonDaDuyet] = useState(0);
    const headers = {
    ...
    const ngayhn = new Date();  // Ngày hôm nay

    const isDateValid = (ngayNhan) => {
        // Chuyển đổi ngày nhận và ngày hiện tại thành đối tượng Date chỉ chứa ngày (không có giờ)
        const dateNhan = new Date(ngayNhan);
        const today = new Date();

        // Đặt giờ của cả hai ngày là 00:00 để loại bỏ ảnh hưởng của giờ, phút, giây
        dateNhan.setHours(0, 0, 0, 0);
        today.setHours(0, 0, 0, 0);

        // Tính sự khác biệt giữa ngày hiện tại và ngày nhận
        const timeDifference = today - dateNhan;

        // Chuyển đổi sự khác biệt thành ngày
        const dayDifference = timeDifference / (1000 * 3600 * 24);  // Chuyển mili giây thành ngày

        // Kiểm tra xem sự khác biệt có <= 2 ngày không
        return dayDifference <= 2;
    }; // Chuyển ngày thành định dạng ngày tháng năm
            ...
            try {
                ...
                if (response.status === 200) {
                    ...
                    if (trangThai === "Đã duyệt") {
                        setSoDonDaDuyet(response.data.$values.length);
                        sessionStorage.setItem('soDonDaDuyet', response.data.$values.length);
                    }
                } else {
    ...
    const handleSaveEdit = async () => {
        ...
        const result = await Swal.fire({
            title: "Bạn có cập nhật lại thông tin?",
            showDenyButton: true,
        ...
        if (diachisuane.tinhname && diachisuane.huyenname && diachisuane.xaname && diachisuane.diachict && diachisuane.phiShip) {
            ...
            console.log("dulieugui",dulieugui)
            try {
    ...
    const handleSubmit = async () => {
        ...
        if (result.isConfirmed) {
            if (!lydoHoantra || hinhanh.length === 0) {
                Swal.fire({
                    title: "Lỗi",
                    text: "Vui lòng điền lý do và thêm hình ảnh sản phẩm",
                    icon: "error"
                });
                return;
            }
        ...
        return (
            <div>
                ...
                <Table striped bordered hover>
                    <thead>
                        <tr>
                            <th style={{ textAlign: 'center' }}>Thông tin người nhận</th>
                            {activeTrangThai === "Đã hủy" ? (
                                ...
                                <th style={{ textAlign: 'center' }}>Ngày Giao</th>
                            ) : activeTrangThai === "Đã nhận" ? (
                                <th style={{ textAlign: 'center' }}>Ngày Nhận</th>
                            ) : activeTrangThai === "Hoàn trả" ? (
                                <th style={{ textAlign: 'center' }}>Ngày trả</th>
                            ) : (
                                <th style={{ textAlign: 'center' }}>Ngày Đặt</th>
                            )}
                            {activeTrangThai === "Đã hủy" && <th style={{ textAlign: 'center' }}>Lý do</th>}
                            <th style={{ textAlign: 'center' }}>Tổng Tiền</th>
                            <th style={{ textAlign: 'center' }}>Phi ship</th>
                            <th style={{ textAlign: 'center' }}>Phương thức </th>
                            <th style={{ textAlign: 'center' }}>Thanh toán</th>
                            <th style={{ textAlign: 'center' }}>Tình trạng đơn</th>
                    ...
                    <tbody>
                        {paginatedData.length > 0 ? (
                            paginatedData.map((dh) => (
                                <tr key={dh.idHD} >
                                    <td style={{ width: '400px' }} onClick={() => handleXemChiTiet(dh.idHD)}>
                                        <div>
                                            <strong>ID: {dh.idHD}</strong><br />
                                            <strong>Tên nhận: {dh.tenNguoiNhan}</strong><br />
                                            {/* <strong>Tên tài khoản: {dh.tenTaiKhoan}</strong><br /> */}
                                            SĐT: {dh.sdt}<br />
                                            Địa chỉ: {dh.diaChi}
                                            <br />
                                            Ghi chú: {dh.ghiChu}
                                    ...
                                    {activeTrangThai === "Đã hủy" ? (
                                        <td onClick={() => handleXemChiTiet(dh.idHD)}>{new Date(dh.ngayHuy).toLocaleDateString()}</td>
                                    ) : activeTrangThai === "Đã duyệt" ? (
                                        <td onClick={() => handleXemChiTiet(dh.idHD)}>{new Date(dh.ngayDat).toLocaleDateString()}</td>
                                    ) : activeTrangThai === "Đang giao" ? (
                                        <td onClick={() => handleXemChiTiet(dh.idHD)}>{new Date(dh.ngayGiao).toLocaleDateString()}</td>
                                    ) : activeTrangThai === "Đã nhận" ? (
                                        <td onClick={() => handleXemChiTiet(dh.idHD)}>{new Date(dh.ngayNhan).toLocaleDateString()}</td>
                                    ) : activeTrangThai === "Đã nhận" ? (
                                        <td onClick={() => handleXemChiTiet(dh.idHD)}>{new Date(dh.ngayhoantra).toLocaleDateString()}</td>
                                    ) : (
                                        <td onClick={() => handleXemChiTiet(dh.idHD)}>{new Date(dh.ngayDat).toLocaleDateString()}</td>
                                    )}
                                    ...
                                    <td style={{ textAlign: 'center' }} onClick={() => handleXemChiTiet(dh.idHD)}>{dh.ptThanhToan}</td>
                                    <td style={{ textAlign: 'center' }} onClick={() => handleXemChiTiet(dh.idHD)}>{dh.ttThanhToan}</td>
                                    <td style={{ textAlign: 'center' }} onClick={() => handleXemChiTiet(dh.idHD)}>{dh.tinhTrangHD}</td>
                                    <td>
                                        <Dropdown>
                                            ...
                                            <Dropdown.Menu>
                                                {/* <Dropdown.Item>
                                                    <button
                                                        ...
                                                        onClick={() => handleXemChiTiet(dh.idHD)}
                                                    >
                                                        Xem
                                                    </button>
                                                </Dropdown.Item> */}
                                                {activeTrangThai !== "Đã hủy" && activeTrangThai !== "Đã duyệt" && activeTrangThai !== "Đang giao" && activeTrangThai !== "Đã nhận" && activeTrangThai !== "Hoàn trả" ? (
                                                    <Dropdown.Item>
                                                        <button
                                                ...
                                                {activeTrangThai === "Đã nhận" && isDateValid(dh.ngayNhan) ? (
                                                    <Dropdown.Item>
                                                        <button
                                                            style={{ backgroundColor: '#00CCCC', color: 'white', width: '100%' }}
                                                ...
                                                {activeTrangThai === "Đã hủy" ? (
                                                    <Dropdown.Item>
                                                        <button
                                                            style={{ backgroundColor: '##66FF00', color: 'white', width: '100%' }}
                                                ...
                                                {activeTrangThai !== "Đã nhận" && activeTrangThai !== "Đã hủy" && activeTrangThai !== "Hoàn trả" && loaitk === "1" ? (
                                                    <Dropdown.Item>
                                                        <button
    ...
    return (
        <div className="container mt-4">
            ...
            <Tabs
                ...
                className="mb-3"
            >
                {["Chưa duyệt", "Đã duyệt", "Đang giao", "Đã nhận", "Đã hủy", "Hoàn trả"].map((trangThai, index) => (
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)} size="lg" style={{ marginTop: '50px', marginLeft: '100px' }}>
            ...
            <Modal show={showEditModal} onHide={() => setShowEditModal(false)} style={{ width: '1600px' }} backdrop="static">
                <Modal.Header >
            ...
            {/* model Lý do hủy */}
            <Modal show={showCancelModal} style={{ marginTop: '100px' }} onHide={() => setShowCancelModal(false)}>
                <Modal.Header closeButton>
            ...
            <Modal show={showbackmodal} style={{ marginTop: '100px' }} onHide={() => setShowbackModal(false)}>
                <Modal.Header closeButton>
