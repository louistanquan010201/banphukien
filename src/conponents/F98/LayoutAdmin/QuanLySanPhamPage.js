...
const Quanlysanpham = () => {
    ...
    const filteredProducts = products
        .filter(product =>
            ...
            (trangThai === "TatCa" && product.trangThai === "Đang bán")
        )
        .filter(product =>
            product.tenSanPham.toLowerCase().includes(searchTerm.toLowerCase()) // Lọc theo tên sản phẩm
        )
        .sort((a, b) => b.soLuongDaBan - a.soLuongDaBan);  // Sắp xếp theo SoLuongDaBan từ cao đến thấp
    ...
    const handleInputChange = (e) => {
        ...
        if (regex.test(value)) {
            Swal.fire({
                title: 'Lỗi nhập liệu',
                text: 'Tên loại sản phẩm không được phép chứa ký tự đặc biệt như !@#$%^&*().',
                icon: 'error',
            });
            return; // Ngăn việc cập nhật giá trị không hợp lệ
        }
    ...
    const handleDelete = async (productId) => {
        ...
        const result = await Swal.fire({
            title: "Bạn có muốn ngưng hoạt động sản phẩm không?",
            showDenyButton: true,
            confirmButtonText: "Không Xóa",
            denyButtonText: " Xóa",
        });
    ...
    return (
        <div className={`container ${isFormVisible ? 'blur-background' : ''}`} style={{ width: 1300, marginLeft: 250, backgroundColor: 'white', borderRadius: '15px' }}>
            ...
            `}
                ...
                <dib className="row">
                    ...
                    <div className='col-md-2'>
                        <button onClick={handleAddProduct} className="add-product-header btn btn-primary" style={{ textDecoration: 'none', backgroundColor: 'black', color: 'white', }}>
                            Thêm Sản phẩm
                        </button>
                    </div>
                </dib>
                ...
                <div className="tabs">
                    ...
                    <button
                        className={`tab-btn ${trangThai === "TatCa" ? "active" : ""}`}
                        onClick={() => setTrangThai("TatCa")}
                    >
                        Sản phẩm bán chạy
                    </button>
                </div>
                ...
                <table className="table table-bordered">
                    <thead>
                        <tr>
                            {trangThai === "TatCa" && <th>Top</th>} {/* Chỉ hiển thị cột "Top" khi chọn tab "Tất cả sản phẩm" */}
                            <th>Sản Phẩm</th>
                            <th>Hình Ảnh</th>
                            <th>Giá Bán</th>
                            {trangThai ==="TatCa"&&<th>Số Lượng Đã Bán</th>}
                            {trangThai !== "TatCa" && (
                                <>
                                    <th>Số Lượng</th>
                                    <th>Thương Hiệu</th>
            ...
            {Array.isArray(currentProducts) && currentProducts.length > 0 ? (
                currentProducts.map((product, index) => (
                    <tr key={index}>
                        {trangThai === "TatCa" && <td>Top {index + 1 + (currentPage - 1) * itemsPerPage}</td>} {/* Hiển thị thứ tự top toàn bộ */}
                        ...
                        {trangThai ==="TatCa"&&<td>{product.soLuongDaBan}</td>}
                        {trangThai !== "TatCa" && (
                            <>
                ...
                <tr>
                    <td colSpan={trangThai === "TatCa" ? "5" : "10"}>Không có sản phẩm nào để hiển thị</td>
                </tr>
            )}
        </tbody>
                </table>
